...
echo "nxfs_diffutils_0=${nxfs_diffutils_0}"

echo "TMP=${TMP}"

export PATH=${tar}/bin:${bash}/bin:${patchelf}/bin:${coreutils}/bin
...
echo "stage0 diffutils dir: ${nxfs_diffutils_0}"
...
(cd ${nxfs_diffutils_0} && (tar cf - . | tar xf - -C ${staging}))
...
# ----------------------------------------------------------------
# verify a patched executable runs

${out}/bin/diff3 --version
