...
export PATH="${gcc_wrapper}/bin:${binutils}/bin:${m4}/bin:${file}/bin:${gnumake}/bin:${gawk}/bin:${grep}/bin:${sed}/bin:${tar}/bin:${coreutils}/bin:${findutils}/bin:${diffutils}/bin:${bash}/bin"
...
(cd ${builddir} && ${bash_program} ${src2}/configure --prefix=${out} --host=${target_tuple} --build=${target_tuple} --with-mpfr=${mpfr} --with-gmp=${gmp} CC=nxfs-gcc CFLAGS= LDFLAGS="-Wl,-enable-new-dtags")
