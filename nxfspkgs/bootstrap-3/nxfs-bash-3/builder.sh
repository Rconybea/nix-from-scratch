...
(cd ${builddir} && ${bash_program} ${src2}/configure --prefix=${out} --host=${target_tuple} --build=${target_tuple} --without-bash-malloc bash_cv_strtold_broken=no CFLAGS= LDFLAGS="-Wl,-enable-new-dtags")
