...
export PATH="${gcc_wrapper}/bin:${binutils}/bin:${m4}/bin:${coreutils}/bin:${bash}/bin:${tar}/bin:${gnumake}/bin:${gawk}/bin:${grep}/bin:${sed}/bin:${findutils}/bin:${diffutils}/bin"
...
(cd ${builddir} && ${bash_program} ${src2}/configure --prefix=${out} --host=${target_tuple} --build=${target_tuple} --with-gmp=${gmp} CC=nxfs-gcc CFLAGS= LDFLAGS="-Wl,-enable-new-dtags")
