...
export PATH="${gcc_wrapper}/bin:${perl}/bin:${binutils}/bin:${gnumake}/bin:${gawk}/bin:${grep}/bin:${sed}/bin:${tar}/bin:${coreutils}/bin:${findutils}/bin:${diffutils}/bin:${bash}/bin"
...
(cd ${builddir} && ${bash_program} ${src2}/configure --prefix=${out} --host=${target_tuple} --build=${target_tuple} --enable-hashes=strong,glibc --disable-werror CC=nxfs-gcc CFLAGS="-std=gnu17" LDFLAGS="-Wl,-enable-new-dtags")
