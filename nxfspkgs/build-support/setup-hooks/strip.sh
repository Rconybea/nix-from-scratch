# per nixpkgs/pkgs/build-support/setup-hooks/strip.sh

# Requires:
# - fixupOutputHooks    declared in build-support/make-stdenv/setup.sh,
#                       setup.sh includes this via nativeBuildInputs
# - STRIP, RANLIB       from bintools/binutils wrapper, via that package's
#                       setup hook
...
_doStrip() {
    ...
    local -ra stripCmds=(STRIP)
    #local -ra stripCmds=(STRIP STRIP_FOR_TARGET)
    local -ra ranlibCmds=(RANLIB)
...
stripDirs() {
    ...
    if [ -n "${paths}" ]; then
        ...
        [[ "$exit_code" = 123 || -z "$exit_code" ]] || {
            ...
            # interpolation here (relative to nixpkgs, which uses cat)
            # because sandbox is impaired
            # in nix 2.24.9, where ownership of fd #1 (-> /dev/pts/3)
            # prevents subprocesses from opening fd#3 for themselves.
            #
            # problem is that /dev/pts/3 has nixbld owner and nogroup group ->
            # subprocess can use fd#1 granted by parent, but can't open it for itself.
